...
`);
  ...
  async gatherExperiences(kws, topicTitle) {
    ...
    return scored.sort((a, b) => b.hits - a.hits).slice(0, 8);
  }
...
> \u903B\u8F91\uFF1A\u53EA\u8BA9"\u9884\u6D4B\u8D28\u91CF \u2265 \u5DF2\u9A8C\u8BC1\u7206\u6B3E\u6C34\u5E73"\u7684\u7A3F\u5B50\u81EA\u52A8\u8FDB\u8349\u7A3F\u7BB1\u3002`
  ...
  async fetchWebMaterial(topic) {
    ...
    const blocks = items.slice(0, 8).map((r, i) => `### \u{1F310} \u6765\u6E90 ${i + 1}\uFF1A${String(r.title || "(\u65E0\u6807\u9898)").slice(0, 60)}
> ${r.url}
...
`;
    ...
    new import_obsidian.Notice("\u{1F310} \u79C1\u641C\u6293\u53D6\u7F51\u7EDC\u7D20\u6750\u4E2D\u2026");
