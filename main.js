...
`);
  ...
  _soloFlomo(keep) {
    ...
    try {
      ws.leftSplit && ws.leftSplit.collapse();
    } catch (e) {
    }
    try {
      ws.rightSplit && ws.rightSplit.collapse();
    } catch (e) {
    }
  }
  async activateFlomo() {
    ...
    ws.revealLeaf(leaf);
    this._soloFlomo(leaf);
    window.setTimeout(() => this._soloFlomo(leaf), 400);
    window.setTimeout(() => this._soloFlomo(leaf), 1200);
  }
  refreshFlomo() {
