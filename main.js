...
var BatchStatsModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    const fields = ["reads", "likes", "wows", "shares", "followers"];
    const labels = { reads: "\u9605\u8BFB", likes: "\u8D5E", wows: "\u5728\u770B", shares: "\u8F6C\u53D1", followers: "\u6DA8\u7C89" };
...
`);
  ...
  async applyDefaultLayout(center) {
    ...
    await this.placeInSide(VIEW_CAL, "left");
    await this.placeInSide(VIEW_CHAT, "right");
...
${line}`);
  ...
  startHttpServer() {
    ...
    const srv = http.createServer((req, res) => {
      ...
      const auth = req.headers["x-cos-token"] || (req.url.split("token=")[1] || "").split("&")[0] || "";
      if (token && auth !== token) return send(401, { error: "unauthorized" });
      const path = req.url.split("?")[0];
      if (req.method === "GET" && path === "/health") return send(200, { ok: true, plugin: "content-os" });
      if (req.method !== "POST") return send(404, { error: "use POST" });
      let body = "";
      req.on("data", (ch) => {
        body += ch;
...
${(await this.app.vault.cachedRead(gfp)).slice(0, 2e3)}`;
    }
...
`);
  ...
  async runDailyBatch(manual) {
    ...
    try {
      ...
      const summary = `\u{1F3ED} \u4ECA\u65E5\u6279\u91CF\u6D77\u9009\u5B8C\u6210\uFF1A\u5199 ${results.length} \u7BC7\uFF0C${elites.length} \u7BC7\u8FC7\u7CBE\u54C1\u7EBF\uFF08${elite}\uFF09\uFF0C${this.settings.dailyAutoPush ? `${pushed} \u7BC7\u5DF2\u8FDB\u8349\u7A3F\u7BB1` : "\u672A\u81EA\u52A8\u63A8\u9001"}\u3002
...
\u3010\u4F4E\u8F6C\u53D1\u6587\u7AE0\u3011
${await brief(bottom)}`);
    const rows = byShare.map((p, i) => `| ${i + 1} | ${p.title.slice(0, 22)} | ${p.reads} | ${(p.shareRate * 100).toFixed(1)}% | ${(p.engageRate * 100).toFixed(1)}% | ${p.followers} | ${p.gap === Infinity ? "-" : Math.round(p.gap) + "\xD7"} |`).join("\n");
    const report = `---
...
> \u7531 ${pub.length} \u7BC7\u771F\u5B9E\u53D1\u5E03\u6570\u636E\u63D0\u70BC\uFF0C\u5199\u7A3F\u65F6\u81EA\u52A8\u6CE8\u5165 draft-writer\u3002\u6570\u636E\u8D8A\u591A\u8D8A\u51C6\u3002\u4EBA\u5DE5\u53EF\u6539\u3002
