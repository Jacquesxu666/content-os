...
`);
  ...
  async activateFlomo() {
    const ex = this.app.workspace.getLeavesOfType(VIEW_FLOMO);
    if (ex.length) {
      this.app.workspace.revealLeaf(ex[0]);
    } else {
    ...
    try {
      this.app.workspace.leftSplit && this.app.workspace.leftSplit.collapse();
    } catch (e) {
    }
    try {
      this.app.workspace.rightSplit && this.app.workspace.rightSplit.collapse();
    } catch (e) {
    }
  }
  refreshFlomo() {
