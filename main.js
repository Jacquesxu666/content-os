...
`);
  ...
  async runUnattended(manual) {
    ...
    try {
      ...
      const dfm = draft instanceof import_obsidian.TFile ? this.fm(draft) : {};
...
`);
  ...
  cleanForPublish(raw) {
    ...
    t = t.split("\n").filter((l) => !l.startsWith("> \u26A0\uFE0F AI \u521D\u7A3F")).join("\n");
    for (const mark of ["\n## \u81EA\u68C0", "\n## AI \u81EA\u68C0", "\n## GTM", "\n## \u590D\u76D8", "\n### \u{1F3A8} \u914D\u56FE\u63D0\u793A\u8BCD"]) {
  ...
  async pushWechatDraft(file) {
    ...
    const markdown = this.cleanForPublish(await this.app.vault.read(target));
